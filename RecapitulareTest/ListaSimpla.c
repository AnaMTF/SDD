...
void inserareLaSfarsit(Nod** cap, Produs produs) {
	Nod* nodNou = (Nod*)malloc(sizeof(Nod));
	nodNou->info = produs;
	nodNou->next = NULL; //ptc e utlimul din lista
	if (*cap)
	{
		Nod* nodCurent = *cap;
